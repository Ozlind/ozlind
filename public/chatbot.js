(() => {
  ...
  function normalizeChats(value) {
    ...
    return value.filter(c => c && c.id && Array.isArray(c.messages)).slice(0, 100).map(c => ({
      ...
      messages: c.messages.filter(m => m && ["user","assistant"].includes(m.role)).map(m => ({
        ...
        imageUrl: typeof m.imageUrl === "string" ? m.imageUrl.slice(0,4000) : "",
        imagePrompt: typeof m.imagePrompt === "string" ? m.imagePrompt.slice(0,4000) : "",
        generationMeta: m.generationMeta && typeof m.generationMeta === "object" ? {
          width: Number(m.generationMeta.width) || 0,
          height: Number(m.generationMeta.height) || 0,
          model: String(m.generationMeta.model || "").slice(0,80)
        } : null
      }))
    }));
  }
  ...
  function renderMessages() {
    ...
    chat.messages.forEach((message,index) => {
      ...
      if (message.pendingImage) {
        body = `<div class="image-generating"><span class="thinking-inline">Creating image<span>·</span><span>·</span><span>·</span></span><small>Generating from your prompt…</small></div>`;
      } else if (message.imageUrl) {
        const alt = escapeHTML(message.imagePrompt || "Generated image");
        body = `<div class="generated-image-wrap">
          <img class="generated-image" src="${escapeHTML(message.imageUrl)}" alt="${alt}" loading="eager" referrerpolicy="no-referrer">
          <div class="generated-image-bar">
            <span>OZLIND IMAGE</span>
            <button type="button" data-download-image="${index}">Download</button>
            <button type="button" data-regenerate-image="${index}">Regenerate</button>
          </div>
        </div>`;
      } else if (message.role === "assistant" && generating && index === chat.messages.length - 1 && !message.content) {
      ...
      const attachments = Array.isArray(message.attachments) && message.attachments.length
        ? `<div class="message-attachments">${message.attachments.map(a => `<img src="${escapeHTML(a.dataUrl)}" alt="${escapeHTML(a.name || "Attached image")}" loading="lazy" data-preview-image="${escapeHTML(a.dataUrl)}">`).join("")}</div>`
        : "";
      ...
      article.innerHTML = `<div class="message-meta">${escapeHTML(label)}</div>${attachments}<div class="message-body">${body}</div>${sources}${actions}`;
  ...
  function isImageRequest(text) {
    ...
    if (!t) return false;
    return [
      /\b(create|generate|make|draw|design|render|produce)\b.{0,60}\b(an?\s+)?(image|picture|photo|illustration|wallpaper|poster|logo|artwork|visual)\b/i,
      /\b(image|picture|photo|illustration|wallpaper|poster|logo|artwork)\b.{0,30}\b(create|generate|make|draw|design|render)\b/i,
      /\btext\s+to\s+image\b/i,
      /\b(generate|create|make)\s+(a\s+)?visual\b/i
    ].some(rx => rx.test(t));
  }
  ...
  function extractImagePrompt(text) {
    ...
    return raw
      .replace(/^\s*(please\s+)?(create|generate|make|draw|design|render|produce)\s+(me\s+)?(an?\s+)?(image|picture|photo|illustration|wallpaper|poster|logo|artwork|visual)\s*(of|for|showing|with|:)?\s*/i, "")
      .replace(/^\s*(image|picture|photo)\s*[:\-]\s*/i, "")
      .trim() || raw;
  }

  async function generateImage(chat, assistant, prompt) {
    generating = true;
    controller = new AbortController();
    assistant.pendingImage = true;
    $("#sendBtn")?.classList.add("hidden");
    $("#stopBtn")?.classList.remove("hidden");
    setConnection("busy", "Creating image…");
    renderMessages();
    ...
    try {
      const response = await fetch("/api/image-generate", {
        ...
        body: JSON.stringify({ prompt, width: 1024, height: 1024, model: "gemini" })
      });
      let data = {};
      try { data = await response.json(); } catch {}
      if (!response.ok || !data.success || !data.imageUrl) throw new Error(data.error || "OZLIND could not create that image.");
      assistant.pendingImage = false;
      assistant.imageUrl = data.imageUrl;
      assistant.imagePrompt = data.prompt || prompt;
      assistant.content = "";
      assistant.generationMeta = { width: data.width, height: data.height, model: data.model };
    } catch (e) {
      assistant.pendingImage = false;
      if (e?.name === "AbortError") assistant.content = "Image generation stopped.";
      else { assistant.error = true; assistant.content = e?.message || "OZLIND could not create that image."; toast(assistant.content, "error"); }
    } finally {
      generating = false;
      controller = null;
      ...
      if (!assistant.error) setConnection("ready", "Ready");
    }
  }
  ...
  async function downloadImage(index) {
    ...
    try {
      ...
      link.href = url; link.download = `ozlind-image-${Date.now()}.jpg`;
      document.body.appendChild(link); link.click(); link.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1500);
    } catch { window.open(message.imageUrl, "_blank", "noopener,noreferrer"); }
  }

  function previewImage(src) {
    const overlay = document.createElement("div");
    overlay.className = "image-viewer";
    overlay.innerHTML = `<img src="${escapeHTML(src)}" alt="Image preview"><button class="icon-btn" type="button" aria-label="Close">${icon("i-close","ui-icon")}</button>`;
  ...
  async function send() {
    ...
    if(chat.messages.filter(m=>m.role==="user").length===1) chat.title=titleFrom(text || "Image creation");
    chat.updatedAt=new Date().toISOString(); input.value=""; resizeInput(); persist(); renderMessages();

    if (text && !attachments.length && isImageRequest(text)) {
      const assistant={role:"assistant",content:"",createdAt:new Date().toISOString()};
      chat.messages.push(assistant);
      persist(); renderMessages();
      await generateImage(chat, assistant, extractImagePrompt(text));
      return;
    }
  ...
  async function regenerate(index) {
    ...
    const userText = previousUser.content || assistant.imagePrompt || "";
    chat.messages.splice(index,1); chat.updatedAt=new Date().toISOString(); persist(); renderMessages();
    if (assistant.imageUrl || assistant.imagePrompt) {
      const imageAssistant={role:"assistant",content:"",imagePrompt:extractImagePrompt(userText),createdAt:new Date().toISOString()};
      chat.messages.push(imageAssistant); persist(); renderMessages();
      await generateImage(chat,imageAssistant,extractImagePrompt(userText));
    } else {
      await generate(chat);
    }
  }
  ...
  document.addEventListener("click", event => {
    ...
    const dlImg=event.target.closest("[data-download-image]");if(dlImg){downloadImage(Number(dlImg.dataset.downloadImage));return;}
    const rgImg=event.target.closest("[data-regenerate-image]");if(rgImg){regenerate(Number(rgImg.dataset.regenerateImage));return;}
